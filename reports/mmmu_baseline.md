...
우리 결과는 62.00%(558/900)로 공식 비교값 67.4%보다 5.40%p 낮다. 격차의 주요 원인으로 규칙 기반 채점의 표현 처리 한계와 장문 반복 등 응답 생성 실패를 추정한다.

첫째, Manage_20에서 모델은 정답 2960에 대해 `$2,960 unfavorable`라고 답했으나 비교 규칙이 이를 인정하지 못했다. Qwen 공개 평가 코드는 규칙 추출 실패 시 LLM 보조 판정을 사용할 수 있어 우리의 엄격한 비교와 다르다. 우리는 API 키·과금·외부 서비스 의존도를 줄이고 채점 절차를 재현하기 위해 고정 규칙만 사용했으며, 그 대가로 일부 유효한 표현을 놓쳤다. [공식 채점 방식](https://github.com/QwenLM/Qwen3-VL/blob/96588727e44c78b25ba03ea03b8e12f7e64fd0da/evaluation/mmmu/README.md#custom-evaluation-logic)

둘째, 출력 상한 종료 90건 중 85건이 오답이었다. Accounting_9는 회계 항목의 해석을 반복하다 결론 없이 종료했다. 이는 단순한 추출 누락이 아닌 생성 단계의 실패다. 상한은 공개 설정과 같은 32,768이므로 모든 실패를 출력 예산 부족으로 설명하기 어렵다. 추출 실패 97건 중 76건은 상한 종료와 겹쳐 별도 손실로 합산하지 않는다. 자연 종료 후에도 추출 실패 21건, 추출된 객관식·주관식 답 불일치가 각각 218건·18건 남았다.

셋째, 공개 코드의 엔진 seed 42와 우리의 문항별 seed 정책이 달라 생성 결과가 달라질 수 있다. 동일 채점기에서 이미지 입력 확대 전후 정답은 554개→558개로 증가가 제한적이었다. 전처리·실행환경도 달라 해상도만의 효과로 단정할 수 없다. 따라서 격차는 해상도나 길이 하나보다 최종답 생성의 실패와 채점 기준 차이가 복합적으로 작용한 결과로 해석한다.
<!-- REPORT_DYNAMIC:END -->
...
- **이미지 입력 확대의 효과**: 출력 상한을 모두 32,768토큰으로 두고 같은 최종 파서로 평가했을 때,
  기존 이미지 입력은 554/900(61.56%), 확대 입력은 558/900(62.00%)으로 차이는 4문항(+0.44%p)이었다.
  이번 비교에서는 이미지 토큰 증가가 큰 점수 상승으로 이어지지 않았다. 다만 전처리 경로·문맥 한도·
  실행환경도 달라 해상도만 바꾼 통제 실험은 아니므로, 해상도 확대 자체가 효과 없다고 단정하지 않는다.
- **파서의 표현 처리 한계**: 사전에 정의한 답 선언과 표기만 처리하므로, 의미가 같은 답이라도
  표현이 지원 범위를 벗어나면 놓칠 수 있다. 동의어 추정·일반적인 단위 환산·복잡한 수식 동치 판정은
  지원하지 않으며, 수치에 추가된 설명을 처리하지 못해 정답을 오답으로 판정하는 경우도 남아 있다.
- **엄격한 판정의 절충점**: 복수 후보나 해결되지 않은 답의 충돌은 미확정으로 처리하며,
  기준 정답에 맞춰 중간 풀이의 후보를 선택하지 않는다. 우연한 일치로 오답을 정답 처리하는 경로를
  줄이는 대신, 의도가 정답이더라도 최종 표현이 모호한 응답은 점수로 인정하지 못할 수 있다.
  반복·잘림으로 최종답이 없는 응답을 파서가 대신 완성하지도 않는다.
- **검증 범위**: 기존 validation 응답과 합성 회귀 사례를 이용한 검증이므로,
  독립 데이터에서 모든 정답 표현을 회수하거나 모든 거짓 정답을 차단한다고 보장할 수 없다.
