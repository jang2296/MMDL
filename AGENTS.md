...
> 배포 checkpoint: 코드 `a7fb5fec8d6b2a96c29e4577b351926e1aaba4b9` 게시 후 RTX3090 Pod
> `pdszzbcroakws1`에서 clean checkout·설치·BF16·다운로드를 통과하고 A/B/C6문항씩 진행 중이다.
> 04:55 UTC A4/6 완료, 신규900은 아직 시작하지 않았다. 완료된 결과를 회수·검증하기 전 Pod를 삭제하지 않는다.
> 05:05 UTC 후속 실행을 등록했다. A/B/C18개 완료·오류0·hash·독립 재채점 guard→control archive→
> 동일 commit C full900(`mmdl-input-v2-full`) 순서다. 대기와 실제 full 시작을 구분하고 Pod 로그/상태를 확인한다.
